...
HARD_QUERIES: list[dict] = [
    ...
    {"q": "一堆重复的数据只要保留不重复的那些", "kw": ["去重"]},
    {"q": "键和值成对存的数据怎么一个一个拿出来", "kw": ["字典", "items"]},
    {"q": "函数默认参数有时候会记住上一次调用的值", "kw": ["默认参数", "可变"]},
    {"q": "数据太多不想一次性全装进内存怎么边算边出", "kw": ["生成器", "yield"]},
    {"q": "想给很多函数统一加计时或日志又不想改函数本身", "kw": ["装饰器"]},
    {"q": "代码报错以后怎么让它别崩继续往下跑", "kw": ["except", "异常", "finally"]},
    {"q": "怎么把程序里的处理结果存成一个文本文件", "kw": ["open", "with"]},
    {"q": "大模型怎么用它没见过的私有资料来回答问题", "kw": ["RAG", "检索增强"]},
    {"q": "大模型一本正经地胡说八道有什么办法治", "kw": ["幻觉"]},
    {"q": "上千万条向量怎么快速找出最相似的几条", "kw": ["Milvus", "FAISS", "HNSW"]},
    {"q": "长文档整篇喂给模型效果差该按什么粒度拆开", "kw": ["切块", "chunking", "重叠"]},
    {"q": "怎么写指令让模型的输出更稳定更符合要求", "kw": ["提示词", "prompt", "少样本"]},
    {"q": "对话太长模型记不住前面的内容怎么办", "kw": ["上下文", "token"]},
    {"q": "每个测试都要先准备一套数据怎么写一份就够", "kw": ["fixture"]},
    {"q": "表格数据想按某一列分组求和用什么做", "kw": ["Pandas", "DataFrame", "分组"]},
]
...
HARD2_QUERIES: list[dict] = [
    ...
    {"q": "数组里把重复元素剔掉只留一份", "kw": ["去重"]},
    {"q": "存 key-value 的容器怎么循环遍历取值", "kw": ["字典", "items"]},
    {"q": "为什么给参数设的默认值有时会带着上次的旧数据", "kw": ["默认参数", "可变"]},
    {"q": "序列的结果一个接一个吐出来别等全部算完", "kw": ["生成器", "yield"]},
    {"q": "在函数外面再包一层来做公共逻辑的写法叫什么", "kw": ["装饰器"]},
    {"q": "出错的地方怎么接住并且保证收尾清理一定执行", "kw": ["except", "异常", "finally"]},
    {"q": "读本地一个文本文件的内容到程序里怎么写", "kw": ["open", "with"]},
    {"q": "先去资料库查再把查到的喂给模型回答的架构", "kw": ["RAG", "检索增强"]},
    {"q": "模型瞎编不存在的东西这个问题叫什么怎么缓解", "kw": ["幻觉"]},
    {"q": "海量特征向量之间算相似度用什么基础设施", "kw": ["Milvus", "FAISS", "HNSW"]},
    {"q": "知识入库前切多大切多碎对检索效果最好", "kw": ["切块", "chunking", "重叠"]},
    {"q": "给模型几个例子让它照着样子作答的方法", "kw": ["提示词", "prompt", "少样本"]},
    {"q": "模型一次能吃下的最大字数由什么决定", "kw": ["上下文", "token"]},
    {"q": "测试用例之间共用的前置初始化怎么复用", "kw": ["fixture"]},
    {"q": "csv 表格想做筛选统计聚合用哪个库顺手", "kw": ["Pandas", "DataFrame", "分组"]},
]
