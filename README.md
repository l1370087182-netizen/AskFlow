...
- **学习任务板**：发布学习目标 / 面试结果一键上板 → `planner` 拆解任务树 → 检索知识库编写学习材料；**缺资料自动爬取**补齐入个人知识库，知识资产越用越厚
...
| 知识库 `kb.html` | 全局分类聚合 + 正文弹窗 + 翻译；**个人知识库**：手工增删改、整站浅爬（AI 清洗）、AI 添加（对话式定题自动爬取）、爬取任务实时面板 |
| 模拟面试 `interview.html` | JD + 简历双图上传 → 逐轮追问 → 总评卡片 → 面试记录列表 → 一键生成学习计划上板 |
| 任务板 `learning.html` | 学习目标发布 / 面试上板，任务树进度、材料阅读、取消 |
...
| 知识库 | `/api/knowledge/…` | 列表 / 分类 / 详情 / 上传；`/my/*` 个人库增删改、整站浅爬、AI 添加 |
| 翻译 | `POST /api/translate/knowledge/{id}` | 知识条目翻译 |
| 向量化 | `POST /api/embedding/run` | 触发切块 + 向量化流水线 |
| 检索 | `POST /api/retrieval/search` | 混合检索调试 |
| 对话 | `POST /api/chat`（SSE）、`/history`、`/undo`、`/ping`、`/sessions` | 双模式对话、历史、撤回、模型连通性测试、会话管理 |
...
6. **密钥安全**：`.env` 与 `deploy/` 已被 `.gitignore` 忽略，切勿提交任何真实密钥。
