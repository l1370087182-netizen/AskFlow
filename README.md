...
再由多 Agent 任务引擎消费知识、也生产知识，形成闭环学习系统。
...
费曼模式（AI 当学生）：你当老师讲给 AI 听，它逐轮追问挑毛病，结束后输出总结复述 + 1-10 评分 + 讲对/讲错/遗漏清单，评分自动落库
...
学习任务板：发布学习目标 / 面试结果一键上板 → planner 拆解任务树 → 检索知识库编写学习材料；缺资料自动爬取补齐入个人知识库，知识资产越用越厚
...
任务地基：agent_task 表（MySQL 唯一真相源）+ 两层锁认领（Redis SETNX + DB CAS）+ 幂等写回 + 强制 work_log 审计 + 存储层状态机 + 超时回收器（重试 3 次耗尽即跳过，流水线不挂死）
...
🏗 系统架构
 ...
 知识生产流水线（任务引擎，后台异步，可恢复可审计）：                              │
  ...
  └────────────┘  └──────────────┘  └──────────┬──────────┘
                                               │
       用户提问/讲解 ──▶ BM25 + 向量混合检索 ──▶ Rerank ──▶ LLM 流式生成
                                               │
                             ┌─────────────────┴────────────┐
                             ▼                              ▼
...
Milvus / Milvus Lite（pymilvus）



向量存储与 ANN 检索（Lite 供低内存部署）
...
OpenAI 兼容（httpx）+ Anthropic Messages（官方 SDK）
...
└── src/
    ...
    ├── agent_engine/        # 多 Agent 任务引擎（两层锁/CAS/work_log/超时回收）
    ├── agents/              # producer / reviewer / curator / planner
    ├── ocr/                 # 视觉模型读图（JD/简历）
...
scripts/seed_terms.py



从已入库文章提炼术语灌 tech_term 表（卡片数据源），--enrich 补详情





scripts/seed_ai_knowledge.py



LLM 批量生成 AI/Python 高频知识讲解入库





scripts/probe_llm.py



探测哪套模型配置真正可用（文本 + 图片双通道）





scripts/chat_test_client.py
...
无 Claude 密钥时验证 Anthropic 协议的 mock 服务
...
讲解/费曼双模式、SSE 流式、多会话管理、评分卡片、消息撤回、⚙️ 个人模型配置
...
全局分类聚合 + 正文弹窗 + 翻译；个人知识库：手工增删改、整站浅爬（AI 清洗）、AI 添加（对话式定题自动爬取）、爬取任务实时面板
...
JD + 简历双图上传 → 逐轮追问 → 总评卡片 → 面试记录列表 → 一键生成学习计划上板
...
/api/auth/send-code /register /login /reset /me
...
GET/PUT /api/user/llm



个人模型配置（api_key 加密存储、脱敏返回）
...
列表 / 分类 / 详情 / 上传；/my/* 个人库增删改、整站浅爬、AI 添加
...
POST /api/translate/knowledge/{id}
...
POST /api/chat（SSE）、/history、/undo、/ping、/sessions



双模式对话、历史、撤回、模型连通性测试、会话管理
...
GET /api/card/today、POST /api/card/refresh、GET /api/card/overview
...
POST /api/jd/analyze、GET /api/jd/{id}、GET /api/jd/
...
POST /api/interview/start /answer（SSE）、GET /records、/records/{id}、/records/{id}/plan
...
POST /api/board/goals /from-interview、GET /api/board/、/tasks/{id}、/tasks/{id}/cancel
...
Milvus Lite 单进程独占：配置了 MILVUS_LITE_PATH 时，同一 .db 文件同一时刻只能被一个进程打开。后端运行中不要另起进程跑离线向量化脚本；用进程内的 POST /api/embedding/run 无此问题。混合检索已做降级：向量路不可用自动退化为纯 BM25。



勿用 uvicorn --reload 跑后端：任务引擎（Producer Agent 池 + 超时回收器）随后端启动，多进程会重复消费任务。



AUTH_SECRET_KEY 一经设定勿更换：它同时是用户私有 api_key 的加密派生源，更换会导致存量密文解不出。



本地 Redis 端口：Windows 本机 6379 可能落入 Hyper-V 保留端口段无法监听，可改用其他端口（如 6500）；Linux 部署无此限制。



未配置 SMTP 时：注册/重置密码的验证码直接打印在后端控制台，方便本地开发。



密钥安全：.env 与 deploy/ 已被 .gitignore 忽略，切勿提交任何真实密钥。
