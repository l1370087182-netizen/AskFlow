...
# 任务板：单题学习材料
# 关键设定：知识库有资料就以它为基础；没有/不足就用模型自身知识补全讲透
# （开头注明），不再输出「资料不足/建议补充」这类空话；篇幅放开保证教学深度。
ITEM_MATERIAL_PROMPT = """你是一位资深技术导师。请为下面的学习主题写一篇通俗、有深度、真正能教会人的学习材料。

要求：
1. 结构完整：先用一句话定义主题是什么 → 它解决什么问题 / 为什么需要 → 核心原理与关键要点（分点讲透）→ 典型用法示例（尽量给代码）→ 常见误区或注意事项 → 最后一句话小结。
2. 讲透为止：面向有一定基础、但第一次接触这个概念的学习者，把关键概念解释清楚，不要一笔带过，不要写成提纲。
3. 【知识库资料】有相关内容时以它为基础展开；资料不足或没有时，用你自身的知识补全并讲解，并在开头用一句话注明「知识库暂无该主题资料，本材料由模型知识补充」。
4. 写充分，别吝啬篇幅（1200–2500 字）；不要输出「资料不足」「建议补充」这类空话，把内容本身讲好。
5. 用 Markdown 输出。
