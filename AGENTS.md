...
│   ├── css/style.css
│   └── js/                  # api.js（后端封装）/ home.js / chat.js
└── src/
...
- **tech_term（技术术语表，每日卡片数据源）**：`term`（术语，唯一，如 aigc）、`alias`（别名，逗号分隔）、`category`、`brief`（一句话简介）、`source_url`。由 `scripts/seed_terms.py` 从已入库文章标题 + LLM 辅助提炼，归一化判重（忽略大小写/空格/连字符），可重复执行扩充。
...
**记忆**：会话历史按 `session_id` 以 JSON 存 `sessions/` 目录（每个模式独立上下文）。
...
- `generation/llm.py` 的 `ChatLLM` 支持两种协议，由 `.env` 配置切换，用户可自由填入任意服务商的地址与密钥：
  ...
  - `CHAT_PROVIDER=anthropic`：Anthropic Messages 协议（Codex 官方或中转站），走官方 `anthropic` SDK 的 `messages.stream`
...
- 协议差异在 `ChatLLM` 内部屏蔽，上层（`chain.py` / `chat_controller.py`）不感知：
  - OpenAI 风格的 `messages`（含 system 角色）发给 Anthropic 时自动转换：system 抽成独立参数、连续同角色消息合并（费曼结束轮有连续 user）、`max_tokens` 必填、新 Codex 模型不接受 `temperature` 故 anthropic 路径不传
- 新增依赖：`anthropic`（官方 SDK）；本地无 Codex 密钥时可用 `scripts/mock_anthropic_server.py` 起 mock 服务验证 anthropic 路径
...
| 对话 | POST | `/api/chat` | 双模式对话，SSE 流式（阶段 7） |
...
# 对话大模型（双协议，用户自填接入点）
CHAT_MODEL=...                # 如 Qwen/Qwen2.5-72B-Instruct 或 Codex-opus-4-8
CHAT_KEY=sk-xxx
